...
class MoleculeTransformer(torch.nn.Module):
    def __init__(self, pe_dim, layers, heads, hidden_dim, dropout) -> None:
        super().__init__()

        self.embeddings = torch.nn.Embedding(28, pe_dim, max_norm=1)
    ...
    def forward(self, batch) -> torch.Tensor:
        batch_len = len(batch)
        x = self.embeddings(batch.x) + batch.pe
...
def main(
    ...
    accelerator.init_trackers(
        experiment_name, init_kwargs={"mlflow": {"run_name": run_name}}, config=hparams
    )
    ...
    for epoch in range(epochs):
        ...
        accelerator.log({"lr": float(scheduler.get_last_lr()[0])}, epoch)
        ...
        if epoch % 5 == 0:
            if accelerator.is_main_process:
                ...
                checkpoint_path = unquote(urlparse(run.info.artifact_uri).path)
